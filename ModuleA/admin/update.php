...
if (!empty($_POST)) {
    ...
    if (($isSuccess && $isImageUpdated && $isUploadSuccess) || ($isSuccess && !$isImageUpdated)) {
        $db = Database::connect();
        if ($isImageUpdated) {
            $statement = $db->prepare("UPDATE produit  set pr_nom = ?, pr_description = ?, pr_cout = ?, fk_rayon = ?, pr_image = ? WHERE pr_id = ?");
            $statement->execute(array($name, $description, $price, $category, $image, $id));
        } else {
            $statement = $db->prepare("UPDATE produit  set pr_nom = ?, pr_description = ?, pr_cout = ?, fk_rayon = ? WHERE pr_id = ?");
    ...
    $db = Database::connect();
    $statement = $db->prepare("SELECT * FROM produit where pr_id = ?");
    $statement->execute(array($id));
    $item = $statement->fetch();
    $name           = $item['pr_nom'];
    $description    = $item['pr_description'];
    $price          = $item['pr_cout'];
    $category       = $item['fk_rayon'];
    $image          = $item['pr_image'];
    Database::disconnect();
}
                    ...
                    <div class="form-group">
                        <label for="category">Catégorie:
                            <select class="form-control" id="category" name="category">
                                <?php
                                $db = Database::connect();
                                foreach ($db->query('SELECT * FROM rayon') as $row) {
                                    if ($row['id'] == $category)
                                        echo '<option selected="selected" value="' . $row['ra_id'] . '">' . $row['ra_nom'] . '</option>';
                                    else
                                        echo '<option value="' . $row['ra_id'] . '">' . $row['ra_nom'] . '</option>';;
                                }
                                Database::disconnect();
                                ?>
                            </select>
